...
class Flatten(nn.Module):
    ...
    def forward(self, x):
        #print('flatten:',x.view(x.size(0), -1).shape,x.view(x.size(0), -1).dim())        
...
class ConvBlock(nn.Module):
    def __init__(self, indim, outdim, pool = True,BatchNorm = True, padding = 2):
        super(ConvBlock, self).__init__()
        self.indim  = indim
        self.outdim = outdim
        self.C = Conv1d_fw(indim, outdim, kernel_size=5, padding = padding)
        if BatchNorm:
            self.BN = BatchNorm1d_fw(outdim)
        ...
        ## Initial the weight at the first step
...
class ConvNet(nn.Module):
    def __init__(self, depth,num_feature,flatten = True):
        ...
        for i in range(depth):
            if i==0:
                ...
                indim =960
                outdim = 960
            B = ConvBlock(indim, outdim, pool = ( i <3 ), BatchNorm = True) #only pooling for first 3 layers
            trunk.append(B)
        ...
        ## when the input_feature = 1000
        if num_feature == 1000:
            input_feat_dim = 14400 # 16*960
        elif num_feature == 4000:
            input_feat_dim = 60480 # 63*960
        
        ## Fully connected network
        mid_feat_dim = int(input_feat_dim/4) 
        if num_feature == 1000: ## Two layers 15360 to 3840 then to 1
            L1=Linear_fw(input_feat_dim,mid_feat_dim)
            relu=nn.ReLU()
            self.final_feat_dim = mid_feat_dim
            parametrized_layers = [L1,relu]
            self.trunk = nn.Sequential(*trunk,*parametrized_layers)
        elif num_feature == 4000: ## Three layers 15120 to 3780 to 1
            L1=Linear_fw(input_feat_dim,mid_feat_dim)
            relu1=nn.ReLU()
            self.final_feat_dim = int(mid_feat_dim/4)
            L2=Linear_fw(mid_feat_dim,self.final_feat_dim)
            relu2=nn.ReLU()
            parametrized_layers = [L1,relu1,L2,relu2]
            self.trunk = nn.Sequential(*trunk,*parametrized_layers)
        print(self.trunk)
...
def Conv4(num_feature):
    return ConvNet(4,num_feature)

def Conv6():
    return ConvNet(6)
	
def Conv4NN():
    return ConvNetNoNorm(4)

def Conv6NN():
    return ConvNetNoNorm(6)
